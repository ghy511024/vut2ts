...
export default {
    ...
    toString() {
        let str = `  ${(()=>{if(this.expData('isTrue',this))
{
 var str = "";let _tmpItem; if(this['item']){_tmpItem=this['item']}
this.expData('array',this).forEach(item=>{
this['item'] = item;
str += `<div class='xixi'>${this.expData('item',this)}</div>`
});
this['item']=_tmpItem;
return str;}else if(this.expData('isTrue',this))
{
return `<div class='haha'></div>`
}else(this.expData('isTrue',this))
{
...
return str})()}
  ...
  <div class="${((this.expData('true',this)?'oldClass':'')+' '+(this.expData('isTrue',this)?'name':'')+' '+(this.expData('123',this)?'name2':'')+' '+(this.expData('123',this)?'name2':'')+' '+(this.expData('ghy=="123"',this)?'isghy':'')).replace(/(^\s+|\s+$)/g,'')}">ccc
  </div>
`
        return str
    }
