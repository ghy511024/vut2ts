    ...
    toString() {
        let str = `  <ul>    ${(() => {
            ...
            this.expData('houseList', this).forEach(item => {
                this['item'] = item;
                str += `<li class="${((this.expData('item.id==123', this) ? 'xixi' : '') + ' ' + (this.expData('true', this) ? 'haha' : '')).replace(/S+/g, '')}">      <h1 houseId="${this.expData('item.id', this)}">${this.expData('item.title', this)}</h1>
      ${(() => {
